...
export default function RegisterFormClient({ initialRegistrations, initialQuota }: RegisterFormClientProps) {
  ...
  useEffect(() => {
    if (quotaInfo) {
      const allFull = quotaInfo.software.full && quotaInfo.network.full && quotaInfo.multimedia.full
      ...
      try {
        ...
        if (formData.buktiFollowPdf) {
          ...
          if (!uploadResult.success) {
            setMessage({ type: 'error', text: uploadResult.error || 'Gagal mengupload file' })
            return
          }
        ...
        <Card className="bg-[#0a0a0a] border border-gray-800 shadow-2xl relative overflow-hidden">
          ...
          <CardContent className="space-y-6 pt-6 bg-[#0a0a0a]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                <Label htmlFor="pilihanPelatihan" className="text-xs font-mono text-gray-400">PILIHAN_PELATIHAN *</Label>
                <Select
              ...
              <div className="space-y-2">
                <Label htmlFor="buktiFollowPdf" className="text-xs font-mono text-gray-400">BUKTI_FOLLOW_PDF *</Label>
                <Input
                  id="buktiFollowPdf"
                  required
                  type="file"
                ...
                />
                <p className="text-xs text-gray-500 font-mono">Bukti screenshot follow Instagram <Link href='https://instagram.com/pccpolines' className='text-blue-500 underline' target='_blank'>@pccpolines</Link> dan <Link href='https://instagram.com/workshop.pcc' className='text-blue-500 underline' target='_blank'>@workshop.pcc</Link> dalam satu PDF, maks 2MB.</p>
              </div>
              ...
              >
                {isPending ? 'PROCESSING...' : userRegistration ? 'ALREADY_REGISTERED' : 'SUBMIT_REGISTRATION'}
        ...
        <Card className="mt-8 bg-[#0a0a0a] border border-gray-800">
          <CardHeader className="border-b border-gray-800">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <CardTitle className="text-2xl font-mono text-white">Registration_List</CardTitle>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-gray-800 hover:bg-transparent">
                    <TableHead className="text-gray-400 font-mono text-xs">No</TableHead>
                    <TableHead className="text-gray-400 font-mono text-xs">Nama</TableHead>
                    <TableHead className="text-gray-400 font-mono text-xs">NIM</TableHead>
                    <TableHead className="text-gray-400 font-mono text-xs">Pelatihan</TableHead>
                    <TableHead className="text-gray-400 font-mono text-xs">Status</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {currentRegistrations.map((reg, idx) => (
                    <TableRow key={reg.id} className="border-gray-800 hover:bg-gray-900/50">
                      <TableCell className="font-mono text-gray-400 text-sm">{startIndex + idx + 1}</TableCell>
                      <TableCell className="font-mono text-white text-sm">{reg.namaLengkap}</TableCell>
                      <TableCell className="font-mono text-gray-400 text-sm">{reg.nim}</TableCell>
